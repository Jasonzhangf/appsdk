...
mod tests {
    ...
    fn cli_error_decorates_route_resolve_not_found_from_current_and_legacy_daemons() {
        for error in [
            ...
            assert!(
                formatted.contains(ROUTE_RESOLVE_NOT_FOUND_RECOVERY_GUIDANCE),
                "{formatted}"
            );
            for expected in [
                "`collab down`",
