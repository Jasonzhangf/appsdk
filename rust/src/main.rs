...
fn producer_record_transaction_recover(
    root: &Path,
    module_id: &str,
    input_hash: &str,
...
fn produce_lifecycle_records(root: &Path, module_id: &str, input_path: &str) {
    ...
    if let Some(recovered) = producer_record_transaction_recover(root, module_id, &input_hash) {
        assert_produced_record_shapes(&recovered, module_id);
        println!(
            "{}",
            serde_json::to_string_pretty(&serde_json::json!({
                "ok": true,
...
fn select_bug_read_store(root: &Path, explicit_upstream: bool) -> Result<PathBuf, String> {
    let upstream = resolve_upstream_repo()?;
    if explicit_upstream {
        return Ok(upstream);
    }

    let current = fs::canonicalize(root).unwrap_or_else(|_| root.to_path_buf());
    if current == upstream {
        Ok(root.to_path_buf())
    } else {
        Ok(upstream)
    ...
    I: Iterator<Item = String>,
{
    ...
    match sub.as_str() {
        ...
        "list" | "ls" => {
            ...
            let work_dir =
                select_bug_read_store(root, upstream).unwrap_or_else(|error| fail(error));
