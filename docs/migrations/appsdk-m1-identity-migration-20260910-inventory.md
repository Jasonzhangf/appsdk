...
| `/Users/fanzhang/Documents/github/appsdk/.agent-collab/server/` | Collab | Historical pre-global daemon files, journal, events, logs, PID/socket/lock | retain/Collab migration | Collab owns this historical state. Global Collab persistence is `~/.collab`; use Collab's migration/recovery commands and do not delete or edit this directory manually. |
| `/Users/fanzhang/Documents/github/appsdk/.agent-collab/mailbox/`, `runs/`, `claims/`, `handoff/` | Collab | Historical pre-global collaboration evidence | retain | Preserve JSONL, task, claim, and handoff evidence. Global current state is `~/.collab`; do not truncate or replay by copying. |
